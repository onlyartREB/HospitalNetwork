...
public class PatientTreatment {
  ...
  public void generate(int numPatient, AgentContainer container,
      List<Zone> zones) {
    generating = true;
    ...
    try {
      ...
      for (Zone zone : zones) {
        int nPatZone = (int) Math
            .round(zone.getProportionOfPatients() * numPatient);
        for (int i = 1; i <= nPatZone; i++) {
          String patientName = "Patient" + java.util.UUID.randomUUID() + "_"
              + waitingPatientCounter;
          this.waitingPatientCounter++;
          AgentController patientController = container.createNewAgent(
              patientName, "rampup.Patient", new Object[] {zone});
      ...
      for (Hospital.PatientReceiver patientReceiver : this.patientReceivers
          .keySet()) {
  ...
  public void addNewPatientReceiver(Hospital.PatientReceiver hospital) {
    synchronized (this) {
      ...
      safePrintln(
          "Current size of hospitals that are registred : " + PatientTreatment
              .getInstance(this.mainThread).getPatientReceivers().size());
    }
  }
  ...
  public void
      addFinishedPatientReceiver(Hospital.PatientReceiver patientReceiver) {
    synchronized (this) {
      this.patientReceivers.put(patientReceiver, true);
      if (this.patientReceivers.size() == this.mainThread.getNumbHospital()
          && this.checkIfAllPatientReceiversHaveFinished()) {
        synchronized (this.mainThread) {
          if (this.patientReceivers.size() == this.mainThread.getNumbHospital()
            ...
            safePrintln("Size of hospitals that finished "
                + this.patientReceivers.size());
          }
        }
      }
    }
