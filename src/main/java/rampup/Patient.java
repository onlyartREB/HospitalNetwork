...
public class Patient extends Agent {
  ...
  private void chooseHospital() {
    ...
    for (HospitalData hospital : patientZone.getHospitals()) {
      ...
      System.out.println(
          "Patient " + getLocalName() + " in zone " + patientZone.getZoneIndex()
      ...
      ACLMessage reply;
      do {
        reply = blockingReceive(0); // Check for incoming messages with zero
                                    // timeout
      } while (reply == null);
    ...
    if (!accepted && chosenHospital != null) {
      ...
      String specialHospitalName = "SpecialHospital"; // Change this to the
                                                      // actual name of the
                                                      // special hospital.
      System.out.println("Patient " + getLocalName() + " in zone "
          + patientZone.getZoneIndex() + " chose the Special Hospital.");
      ACLMessage message = new ACLMessage(ACLMessage.REQUEST);
      message.setContent(getLocalName()); // Send the patient's name as content
